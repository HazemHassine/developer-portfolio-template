...
export default function NotFoundGridGame() {
  ...
  useEffect(() => {
    ...
    try {
      const savedGrid = localStorage.getItem('portfolio_404_high_score');
      if (savedGrid) {
      ...
      const savedShooter = localStorage.getItem('portfolio_404_shooter_high_score');
      if (savedShooter) {
        const val = parseInt(savedShooter, 10) || 0;
        setShooterHighScore(val);
        gameRef.current.shooterHighScore = val;
      }
      const savedCallsign = localStorage.getItem('portfolio_404_callsign');
      if (savedCallsign) {
  ...
  const submitScore = async (mode, scoreVal, sectorVal = 1) => {
    ...
    try {
      ...
      try {
        localStorage.setItem('portfolio_404_callsign', cleanCallsign);
      } catch (_) {}
          ...
          if (g.state === 'playing') {
            ...
            if (currentTime - lastHudUpdate > 200) {
              ...
              if (g.score > g.highScore) {
                ...
                try {
                  localStorage.setItem('portfolio_404_high_score', g.score.toString());
                } catch (_) {}
              }
            }
          }
              ...
              if (g.shooterScore > g.shooterHighScore) {
                ...
                try {
                  localStorage.setItem(
                    'portfolio_404_shooter_high_score',
