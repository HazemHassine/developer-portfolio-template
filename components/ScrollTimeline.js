...
export default function ScrollTimeline({ id = 'timeline', experience = [], education = [] }) {
  ...
  const formattedExperience = experience.map((item) => ({
    ...
    type: item.type?.toUpperCase() || 'WORK',
  }));
  ...
  // Combined reverse-chronological list. The first four-digit year keeps this
  // generic for any experience and education content supplied by the owner.
  const combinedList = [...formattedExperience, ...formattedEducation].sort((a, b) => {
